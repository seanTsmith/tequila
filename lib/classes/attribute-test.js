...
test.runnerAttribute = function () {
  test.heading('Attribute Class', function () {
    ...
    test.heading('CONSTRUCTOR', function () {
      ...
      test.heading('Attribute.ID', function () {
        test.paragraph('Attribute also defines the Attribute.ID constructor for ID types.');
        test.example('objects created should be an instance of Attribute.ID', true, function () {
          return new Attribute.ID() instanceof Attribute.ID;
        });
        test.example('should make sure new operator used', Error('new operator required'), function () {
          Attribute.ID();
        });
        test.example('value stores ID and defaults to null', null, function () {
          return new Attribute.ID().value;
        });
        test.example('value can be set in constructor', 123, function () {
          return new Attribute.ID(123).value;
        });
        test.example('toString is more descriptive', "ID('shiny') : ID(1) : ID(null)", function () {
          return new Attribute.ID('shiny').toString() + ' : ' +
            new Attribute.ID(1).toString() + ' : ' +
            new Attribute.ID().toString();
        });
      });
    ...
    test.heading('PROPERTIES', function () {
      ...
      test.heading('value', function () {
        ...
        test.example('should accept assignment of correct type and validate incorrect attributeTypes',
          '8 correct assignments 120 errors thrown', function () {
            ...
            var myValues = [new Attribute.ID(1), 'Jane Doe', new Date, true, 18, new Attribute.ModelID(new Model()), [], myTable];
