...
function Command(/* does this matter */ args) {
  ...
  this._eventListeners = []; // todo- could be faster if all events a property then contents is array of subscribers but may be overkill
}
/*
 * Methods
 */
...
Command.prototype.onEvent = function (events, callback) {
  if (!(events instanceof Array)) {
    if (!(events instanceof String)) throw new Error('subscription string or array required');
...
Command.prototype.emitEvent = function (event) {
  ...
  for (i in this._eventListeners) {
    ...
    if (T.contains(subscriber.events, event)) {
