...
function Command(args) {
  ...
  switch (this.type) {
    ...
    case 'Procedure':
      ...
      for (i in this.contents) {
        if (typeof this.contents[i] != 'object') throw new Error('contents must be array of menu items');
        unusedProperties = T.getInvalidProperties(this.contents[i], ['label', 'command', 'requires', 'timeout']);
        var badJooJoo = [];
        for (j = 0; j < unusedProperties.length; j++) badJooJoo.push('invalid property: ' + unusedProperties[j]);
        if (badJooJoo.length > 1) throw new Error('error creating Command: multiple errors');
        if (badJooJoo.length) throw new Error('contents step(' + i + ') has ' + badJooJoo[0]);
        if (!(typeof this.contents[i].command instanceof Command))
          throw new Error('contents step(' + i + ') must have valid command property set to Command');
        this.contents[i].started = false;   // initial values before executing
        this.contents[i].completed = false;
        this.contents[i].results = null;
      }
      break;
  }
}
