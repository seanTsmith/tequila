...
Bootstrap3PanelInterface.prototype.registerAppBar = function (self, panelBody, panelTitle) {
  ...
  $.get('text/find.md', function (data) {
    panelBodyInsert2.innerHTML = '<div class="well-fucking-well">' + markdown.toHTML(data) + '</div>';
  });
  ...
  $.get('text/bookmarks.md', function (data) {
    panelBodyInsert3.innerHTML = '<div class="well-fucking-well">' + markdown.toHTML(data) + '</div>';
  });
  ...
  $.get('text/history.md', function (data) {
    panelBodyInsert4.innerHTML = '<div class="well-fucking-well">' + markdown.toHTML(data) + '</div>';
  });
  ...
  $.get('text/info.md', function (data) {
    panelBodyInsert5.innerHTML = '<div class="well-fucking-well">' + markdown.toHTML(data) + '</div>';
  });
...
// -------------------------------------------------------------------------------------------------------------------
Bootstrap3PanelInterface.prototype.renderAppBarListItem = function (parent, action, icon, className) {
  ...
  var self = this;
  var html;
  icon = icon || 'fa-square-o';
  className = className || 'btn-default';

  var anchor = document.createElement('a');
  anchor.className = 'btn ' + className + ' ti-shortcut-button';
  ...
  $(anchor).click(function (e) {
    if (action.type == 'Menu' || (action.type == 'Stub' && action.name == 'Back')) {
      ...
      for (menuItem in menuContents) {
        if (menuContents[menuItem].type == 'Menu')
          self.renderAppBarListItem(parent, menuContents[menuItem], 'fa-th-large');
        else
          self.renderAppBarListItem(parent, menuContents[menuItem]);
      }
      if (!(action.type == 'Stub' && action.name == 'Back')) {
        var backCommand = new Command({name: 'Back', description: 'Back to previous selections'});
