...
Bootstrap3PanelInterface.prototype.renderNavBar = function () {
  //
  this.separatorSeen = false; // '-' to right justify items
...
//  this.navBar.className = "navbar navbar-fixed-top";
  ...
  // Navbar toggle button when small presentation (IS LAME)
...
//  this.navBarToggle.innerHTML = '<a href="javascript:app.getInterface().homePanel()">' + iconBar + iconBar + iconBar + '</a>';
//  this.navResponsiveHeader.appendChild(this.navBarToggle);
  ...
  this.navBarBrand.innerHTML = '<a href="javascript:app.getInterface().homePanel()">' + app.get('brand') + '</a>';
  ...
  // All Permanent Fixtures loaded now load dynamic content
...
// -------------------------------------------------------------------------------------------------------------------
Bootstrap3PanelInterface.prototype.renderNavBarPresentation = function () {
  ...
  for (menuItem in menuContents) {
    if (menuContents[menuItem].type == 'Menu') {
      var parentMenu = this.renderNavBarListMenu(this.navList, menuContents[menuItem].name);
      ...
      else if (this.separatorSeen)
        this.renderNavBarListItem(this.navListRight, menuContents[menuItem]);
      else
        this.renderNavBarListItem(this.navList, menuContents[menuItem]);
    }
  }
...
Bootstrap3PanelInterface.prototype.navPicked = function (label) {
  ...
  // TODO remove this app specific code ... wtf ?
  var action = {};
  action.label = label;
  switch (label) {
    case 'Contacts':
