...
Bootstrap3PanelInterface.prototype.dispatch = function (request, response) {
  ...
  if (!this.application || !this.application.dispatch(request)) { // todo application not defined ? review design on this interface / app relationship
    if (request.type == 'Command' && request.command.type == 'Stub')
    ...
    else if (request.type == 'Command' && request.command.type == 'Presentation') {
      ...
      this.renderPanel({label: request.command.name, type: 'presentation', style: 'info', icon: 'fa-building', request: request});
    }
    else {
